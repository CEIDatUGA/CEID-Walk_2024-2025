...
forecast_dates <- Sys.getenv("fdt") %>% 
  (\(x) if (x == "") Sys.Date() else as.Date(x))()
...
rw_forecast <- function(df, fdt, npaths = 100, 
  ...
  ## remove observations more that 2 sds away from mean
...
Getting data online. [not in use currently]

```{r}
#URL <- "https://data.cdc.gov/resource/mpgq-jmmr.csv?$limit=30000"
#preliminary_NHSN_data <- RSocrata::read.socrata(URL)
#last_updated <- gsub("-","",Sys.Date())
#write_csv(preliminary_NHSN_data, paste0(
#  "weekly_forecasts/preliminary_data/Weekly_Hospital_Respiratory_Data__HRD__Metrics_by_Jurisdiction__National_Healthcare_Safety_Network__NHSN___Preliminary__", last_updated, ".csv")
#  )
```

You need to download preliminary NHSN data from this website every week:
...
NHSN_data <- read_csv("weekly_forecasts/preliminary_data/Weekly_Hospital_Respiratory_Data_(HRD)_Metrics_by_Jurisdiction,_National_Healthcare_Safety_Network_(NHSN)_(Preliminary)_20260218.csv") %>% 
  as_tibble() %>% 
...
Final dataset used on the forecasting.
...
```{r}
forecast_date_for_saving<-combined_results$reference_date[1]

forecast_date_for_saving

# Create filename using the date
file_name <- paste0(
  "weekly_forecasts/UGA_CEID-Walk/",
  forecast_date_for_saving,
  "-UGA_CEID-Walk.csv"
)

# Save CSV
write_csv(combined_results, file = file_name)
```
